...
const AppLayout: React.FC<LayoutProps> = (props: LayoutProps) => {
	...
	return (
		<Box display='flex' height='100%' sx={{ backgroundColor: "#FAFBFF" }}>
			{!isAuthenticatedUser && !urlElements.includes('auth') && (
				<Header open={open} setOpen={setOpen} />
			)}
			{isSidebarVisible && !urlElements.includes('main') && !urlElements.includes('auth') && (
			...
			>
				<Box>
					{!urlElements.includes('auth') && <Header open={open} setOpen={setOpen} />}
					{isGLoading && (
						<Box position='absolute' zIndex={10} top={0} bottom={0} left={0} right={0} bgcolor='rgba(255, 255, 255, 0.6)' />
					)}
					{children}
				</Box>
			</Box>
