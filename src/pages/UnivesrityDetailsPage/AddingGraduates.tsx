...
const AddingGraduates: React.FC = () => {
  ...
  const [hasCurrentGeneration, setHasCurrentGeneration] = useState(false);

  const lang = useSelector(selectLanguage);
  const [file, setFile] = useState<File | null>(null);
  const fileInputRef = useRef<HTMLInputElement | null>(null);
  const [archiveLink2, setArchiveLink] = useState<string>("");
  ...
  // }, [archiveLink, !archiveLink2]);
  React.useEffect(() => {
    if (generationProgress >= 100 && genHash) {
      setArchiveLink(`https://generator.ediploma.kz/get-file/archives/${ genHash }.zip`);
      console.log("Generation completed effect triggered");
    }

  }, [generationProgress >= 100 && genHash]);
  React.useEffect(() => {
    // Создаем функцию с доступом к текущим значениям

    const checkStatus = () => {
      if (generationProgress && generationProgress >= 100 && hasCurrentGeneration) {
        return;
      }

      axios.get('https://generator.ediploma.kz/generation_status/' + userState.university_id)
      .then((response) => {
        setStepId(steps[ 1 ].id);
        setType('excel');
        setProgress(2);
        setFile(new File([], 'uploadedFile'));
        setHasCurrentGeneration(true);

        const progress = response.data.progress;
        const max_progress = response.data.max_progress;
        const currentProgress = Math.round((progress / max_progress) * 100);
        setGenerationProgress(currentProgress);
        setGenHash(response.data.generation_hash);
        if (currentProgress >= 100) {
          console.log("Generation completed");
          setArchiveLink(`https://generator.ediploma.kz/get-file/archives/${ response.data.generation_hash }.zip`);
        }

        console.log("Generation progress:", progress, max_progress, currentProgress, hasCurrentGeneration);
      })
      .catch((err) => {
    ...
    checkStatus();
    console.log("useEffect for generation status called");
  ...
  }, [userState, generationProgress, hasCurrentGeneration]); // Добавляем зависимости
  React.useEffect(() => {
    if (isUploaded && archiveLink2) {
      goForward();
    }
  }, [isUploaded]);
      ...
      <Box sx={ {
        ...
        <Box sx={ {
          ...
          { stepId === 2 && (file || type == 'api') && (
            // { stepId === 2 && (file || type == 'api') || hasCurrentGeneration && (
            <Box sx={ { marginTop: 4, marginBottom: '20px' } }>
              ...
              <Box sx={ {
                ...
                <Box sx={ {
                  ...
                  <Box ml="auto">
                    { archiveLink2 && archiveLink2.trim() != "" || type == "api" ?
                      <Button variant="contained" color="primary"
                              sx={ { borderRadius: '1rem', padding: ".75rem" } }
                              onClick={ () => {
                                handleLink(type == "api" ? "https://generator.ediploma.kz/get-file/archives/3.zip" : archiveLink2);
                              } }><DownloadIcon style={ { filter: "brightness(20)" } }/></Button>
                      : (<Box display={ "flex" }><Typography ml="auto" mr=".4rem" mt=".2rem" variant="subtitle1" sx={ {
                        color: '#000000',
                        '@media (max-width: 998px)': {
                          fontSize: '1rem',
                        },
                      } }>{ generationProgress }%</Typography><CircularProgress
                        value={ generationProgress } variant={ 'determinate' } style={ { marginLeft: "auto" } }/></Box>)
                    }
                  </Box>
                </Box>
              </Box>
