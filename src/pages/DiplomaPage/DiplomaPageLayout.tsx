...
export const DiplomaPageLayout: React.FC = () => {
  ...
  const getSpeciality = (e: any) => {
    if (lang === 'ru') {
      let speciality = e.speciality_ru?.substring(e.speciality_ru.search("«"), e.speciality_ru.search("»") + 1).trim();
      return speciality?.length ? speciality : e.speciality_ru;
    }
    if (lang === 'kz') {
      let speciality = e.speciality_kz?.substring(e.speciality_kz.search("«"), e.speciality_kz.search("»") + 1).trim();
      return speciality?.length ? speciality : e.speciality_kz;
    }
    if (lang === 'en') {
      let speciality = e.speciality_en?.substring(e.speciality_en.search("«"), e.speciality_en.search("»") + 1).trim();
      return speciality?.length ? speciality : e.speciality_en;
    }
    return "";
  };
                ...
                <CardContent
                  ...
                  <Box>
                    <Box display='flex' justifyContent='space-between' alignItems='center'>
                      ...
                      <Typography
                        sx={ { fontWeight: '600', fontSize: '16px' } }>{ unis[ lang ][ e.university_id ] }</Typography>
